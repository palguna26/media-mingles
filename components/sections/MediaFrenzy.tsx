...
export function MediaFrenzy() {
  ...
  useLayoutEffect(() => {
    const context = gsap.context(() => {
      ...
      mm.add("(prefers-reduced-motion: no-preference)", () => {
        ...
        gsap.timeline({ scrollTrigger: { trigger: root.current, start: "top 88%", end: "top 24%", scrub: .55 } })
          .fromTo(".campaigns__head>*", { y: 38, opacity: 0 }, { y: 0, opacity: 1, stagger: .07, duration: .35, ease: "power3.out" })
          .fromTo(cards, { y: 120, opacity: 0, rotate: (index) => index % 2 ? 3 : -3, scale: .93 }, { y: 0, opacity: 1, rotate: 0, scale: 1, stagger: .08, duration: .55, ease: "power3.out" }, .14);
      });
      return () => mm.revert();
    }, root);
  ...
  return <section ref={root} className="campaigns" aria-labelledby="campaigns-title"><header className="campaigns__head"><span>Creative output</span><h2 id="campaigns-title">Every service, shot as a story.</h2><p>Each campaign below is one of our services in practice — strategy, production and distribution handled by the same room.</p></header><div className="campaigns__grid">{campaigns.map((campaign) => <div className="campaign-card-shell" key={campaign.image}><article className="campaign-card" data-cursor="VIEW"><Image src={campaign.image} alt={`${campaign.title} campaign poster`} fill sizes="(max-width: 767px) 82vw, (max-width: 1100px) 44vw, 24vw" /><div className="campaign-card__shade" /><div className="campaign-card__copy"><span>{campaign.service}</span><h3>{campaign.title}</h3><p>{campaign.description}</p></div></article></div>)}</div></section>;
}
