...
export function Navbar() {
  ...
  return <><header className={`nav ${scrolled ? "nav--scrolled" : ""}`}><Link href="/" className="wordmark" aria-label="Media Mingles home">MEDIA MINGLES<span>.</span></Link><nav className="nav__links" aria-label="Primary">{navigation.map(item => <Link key={item.href} href={item.href}>{item.label}</Link>)}<Link href="/contact#audit" className="button button--small">Request free audit</Link></nav><button ref={toggle} className="nav__toggle" onClick={() => setOpen(true)} aria-expanded={open} aria-controls="mobile-menu" aria-label="Open menu"><Menu /></button></header><aside className="utility-rail" aria-label="Social links"><a href={socialLinks.instagram}>IG</a><a href={socialLinks.linkedin}>IN</a><a href={`mailto:${contactDetails.email}`}>@</a></aside><div id="mobile-menu" className={`menu ${open ? "menu--open" : ""}`} aria-hidden={!open}><button ref={close} onClick={() => setOpen(false)} aria-label="Close menu"><X /></button>{navigation.map((item, i) => <Link tabIndex={open ? 0 : -1} key={item.href} href={item.href} onClick={() => setOpen(false)}><span>0{i + 1}</span>{item.label}</Link>)}<Link tabIndex={open ? 0 : -1} href="/contact#audit" onClick={() => setOpen(false)}>Request free audit</Link></div></>;
}
